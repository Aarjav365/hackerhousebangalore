...
export default function App() {
  ...
  const renderApply = () => (
    <div className="max-w-3xl mx-auto pt-20 px-6 pb-32">
      <FadeIn>
        <button
          onClick={() => { setView('landing'); setSelectedRole(null); }}
          className="font-sans text-[10px] uppercase tracking-widest text-subtle dark:text-dark-subtle hover:text-ink dark:hover:text-dark-ink transition-colors mb-12"
        >
          ← Back
        </button>
          ...
          {activeRole && (
            ...
            >
              ...
              <p className="font-serif text-base italic text-subtle dark:text-dark-subtle mb-3">
                {activeRole.key === 'software' && 'Pick one of the following directions, or invent your own:'}
                {activeRole.key === 'design' && 'Examples of what a strong response might look like:'}
                {activeRole.key === 'bd' && 'Identify one person who fits the following profile:'}
              </p>
  ...
  return (
    <div className="min-h-screen bg-paper dark:bg-dark-paper text-ink dark:text-dark-ink selection:bg-gray-200 dark:selection:bg-gray-800 transition-colors duration-300">
      ...
      >
        <button
          onClick={() => { setView('landing'); setSelectedRole(null); }}
          className="font-serif text-xl italic font-semibold tracking-tight hover:opacity-80 transition-opacity"
        >
          Hackerhouse v1
        </button>
        <button
      ...
      <main className="relative z-10">
        {renderReferModal()}
        {view === 'landing' ? renderLanding() : renderApply()}
      </main>
